...
// UpsertDailyRollupsFor 把 targetDate 当天（UTC 日 [day, day+1d)）的明细
...
//     重复执行只会用最新统计覆盖；
//   - bucket_date 用 Go 侧算出的 UTC 日期字符串（$1='YYYY-MM-DD'），'...'::date 与
//     DB session timezone 无关；明细窗口用显式 UTC 时间边界 [$2,$3)（timestamptz 绝对比较），
//     确保无论 DB 会话时区为何都按 UTC 日切分，不会把数据聚到相邻日。
func (r *channelMonitorRepository) UpsertDailyRollupsFor(ctx context.Context, targetDate time.Time) (int64, error) {
	...
	const q = `
		...
		ON CONFLICT (monitor_id, model, bucket_date) DO UPDATE SET
		    ...
		    computed_at         = NOW()
	`
	res, err := r.db.ExecContext(ctx, q, dayStr, day, dayEnd)
	if err != nil {
		return 0, fmt.Errorf("upsert daily rollups for %s: %w", dayStr, err)
	}
...
func (r *channelMonitorRepository) DeleteRollupsBefore(ctx context.Context, beforeDate time.Time) (int64, error) {
	// 传 UTC 日期字符串，'...'::date 与 DB session timezone 无关，与 bucket_date 口径一致。
	return deleteChannelMonitorBatched(ctx, r.db, channelMonitorPruneRollupSQL, beforeDate.UTC().Format("2006-01-02"))
}
...
// cutoff 由调用方按列类型传入：明细用 time.Time 对 TIMESTAMPTZ 绝对比较；
// rollup 用 UTC 日期字符串 'YYYY-MM-DD' 对 DATE 列（'...'::date 与 session timezone 无关）。
func deleteChannelMonitorBatched(ctx context.Context, db *sql.DB, query string, cutoff any) (int64, error) {
	var total int64
	for {
...
// 传 Go 侧算出的 UTC 日期字符串（$1='YYYY-MM-DD'），'...'::date 与 DB session timezone 无关，
// 与 UpsertDailyRollupsFor 的 bucket_date 口径保持一致。
func (r *channelMonitorRepository) UpdateAggregationWatermark(ctx context.Context, date time.Time) error {
	const q = `
		...
		ON CONFLICT (id) DO UPDATE SET
		    ...
		    updated_at           = NOW()
	`
	if _, err := r.db.ExecContext(ctx, q, date.UTC().Format("2006-01-02")); err != nil {
