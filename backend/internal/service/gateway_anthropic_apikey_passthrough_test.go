...
func TestGatewayService_AnthropicAPIKeyPassthrough_ForwardStripsCacheControlWhenPromptCachingDisabled(t *testing.T) {
	...
	body := []byte(`{"model":"claude-3-7-sonnet-20250219","stream":false,"system":[{"type":"text","text":"sys","cache_control":{"type":"ephemeral"}}],"messages":[{"role":"user","content":[{"type":"text","text":"hello","cache_control":{"type":"ephemeral"}}]}]}`)
	parsed := &ParsedRequest{
		Body:   body,
		Model:  "claude-3-7-sonnet-20250219",
		Stream: false,
	...
	upstream := &anthropicHTTPUpstreamRecorder{
		resp: &http.Response{
			StatusCode: http.StatusOK,
			Header:     http.Header{"Content-Type": []string{"application/json"}},
			Body:       io.NopCloser(strings.NewReader(`{"id":"msg_test","type":"message","usage":{"input_tokens":1,"output_tokens":1}}`)),
		},
	}
	...
	result, err := svc.Forward(newAnthropicGroupContextForTest(false), c, newAnthropicAPIKeyAccountForTest(), parsed)
	require.NoError(t, err)
	require.NotNil(t, result)
...
func TestGatewayService_AnthropicAPIKeyPassthrough_CountTokensStripsCacheControlWhenPromptCachingDisabled(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	err := svc.ForwardCountTokens(newAnthropicGroupContextForTest(false), c, newAnthropicAPIKeyAccountForTest(), parsed)
	require.NoError(t, err)
	require.Zero(t, strings.Count(string(upstream.lastBody), `"cache_control"`))
	require.False(t, gjson.GetBytes(upstream.lastBody, "system.0.cache_control").Exists())
	require.False(t, gjson.GetBytes(upstream.lastBody, "messages.0.content.0.cache_control").Exists())
