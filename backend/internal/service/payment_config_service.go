...
func defaultPaymentConfig() *PaymentConfig {
	return &PaymentConfig{
		...
		CancelRateLimitMax:        10,
		CancelRateLimitWindow:     1,
		CancelRateLimitUnit:       "",
		CancelRateLimitMode:       "",
	}
}
...
func (s *PaymentConfigService) parsePaymentConfig(vals map[string]string) *PaymentConfig {
	...
	cfg.Enabled = vals[SettingPaymentEnabled] == "true"
	cfg.MinAmount = pcParseFloat(vals[SettingMinRechargeAmount], cfg.MinAmount)
	cfg.MaxAmount = pcParseFloat(vals[SettingMaxRechargeAmount], cfg.MaxAmount)
	cfg.DailyLimit = pcParseFloat(vals[SettingDailyRechargeLimit], cfg.DailyLimit)
	cfg.OrderTimeoutMin = pcParseInt(vals[SettingOrderTimeoutMinutes], cfg.OrderTimeoutMin)
	cfg.MaxPendingOrders = pcParseInt(vals[SettingMaxPendingOrders], cfg.MaxPendingOrders)
	cfg.BalanceDisabled = vals[SettingBalancePayDisabled] == "true"
	cfg.BalanceRechargeMultiplier = normalizeBalanceRechargeMultiplier(pcParseFloat(vals[SettingBalanceRechargeMult], cfg.BalanceRechargeMultiplier))
	cfg.RechargeFeeRate = pcParseFloat(vals[SettingRechargeFeeRate], cfg.RechargeFeeRate)
	if v := vals[SettingLoadBalanceStrategy]; v != "" {
		cfg.LoadBalanceStrategy = v
	}
	cfg.ProductNamePrefix = vals[SettingProductNamePrefix]
	cfg.ProductNameSuffix = vals[SettingProductNameSuffix]
	cfg.HelpImageURL = vals[SettingHelpImageURL]
	cfg.HelpText = vals[SettingHelpText]
	cfg.CancelRateLimitEnabled = vals[SettingCancelRateLimitOn] == "true"
	cfg.CancelRateLimitMax = pcParseInt(vals[SettingCancelRateLimitMax], cfg.CancelRateLimitMax)
	cfg.CancelRateLimitWindow = pcParseInt(vals[SettingCancelWindowSize], cfg.CancelRateLimitWindow)
	cfg.CancelRateLimitUnit = vals[SettingCancelWindowUnit]
	cfg.CancelRateLimitMode = vals[SettingCancelWindowMode]
	if raw := vals[SettingEnabledPaymentTypes]; raw != "" {
		cfg.EnabledTypes = cfg.EnabledTypes[:0]
