...
var (
	...
	ErrChannelMonitorEncryptionKeyNotConfigured = infraerrors.BadRequest(
		"CHANNEL_MONITOR_ENCRYPTION_KEY_NOT_CONFIGURED",
		"a fixed TOTP encryption key (totp.encryption_key) must be configured before creating or updating channel monitor API keys; otherwise stored keys become unreadable after restart",
	)
)
