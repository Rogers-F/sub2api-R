...
type ChannelMonitorService struct {
	...
	// encryptionKeyConfigured 表示部署是否配置了固定的 TOTP 加密主密钥。
	// 为 false 时（密钥在启动时随机自动生成且不落库）写入的 api_key 密文将在下次
	// 重启后无法解密，因此所有写 api_key 密文的路径（Create / Update api_key）必须前置拒绝，
	// 避免静默写入不可恢复的数据。
...
// encryptionKeyConfigured 来自 cfg.Totp.EncryptionKeyConfigured：false 时禁止写入 api_key 密文。
func NewChannelMonitorService(repo ChannelMonitorRepository, encryptor SecretEncryptor, encryptionKeyConfigured bool) *ChannelMonitorService {
	return &ChannelMonitorService{repo: repo, encryptor: encryptor, encryptionKeyConfigured: encryptionKeyConfigured}
}
...
func (s *ChannelMonitorService) Create(ctx context.Context, p ChannelMonitorCreateParams) (*ChannelMonitor, error) {
	...
	if !s.encryptionKeyConfigured {
		return nil, ErrChannelMonitorEncryptionKeyNotConfigured
	}
...
func (s *ChannelMonitorService) applyAPIKeyUpdate(existing *ChannelMonitor, raw *string) (plain string, updated bool, err error) {
	...
	if !s.encryptionKeyConfigured {
		return "", false, ErrChannelMonitorEncryptionKeyNotConfigured
	}
