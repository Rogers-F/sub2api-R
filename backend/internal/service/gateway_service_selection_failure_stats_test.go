...
func TestDiagnoseSelectionFailure_OpenAIUnschedulableDetail(t *testing.T) {
	svc := &GatewayService{}
	acc := &Account{
		ID:          7,
		Platform:    PlatformOpenAI,
	...
	diagnosis := svc.diagnoseSelectionFailure(context.Background(), acc, "gpt-5", PlatformOpenAI, map[int64]struct{}{}, false)
...
func TestDiagnoseSelectionFailure_OpenAIModelRateLimitedDetail(t *testing.T) {
	svc := &GatewayService{}
	model := "gpt-5"
