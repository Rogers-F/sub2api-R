...
func newSSRFSafeHTTPClient(timeout time.Duration) *http.Client {
	tr := &http.Transport{
		// Proxy 显式置 nil：绝不使用环境变量代理（HTTP(S)_PROXY/NO_PROXY）。
		// 若经代理出网，safeDialContext 只能校验到代理 IP，真实目标由代理解析，
		// 会绕过 SSRF 目标校验。监控一律直连 + dial 层校验真实 IP。
		Proxy:                 nil,
	...
	return &http.Client{
		Timeout:   timeout,
		Transport: tr,
		// 不跟随重定向：收到 3xx 即作为最终响应返回（checker 把非 2xx 视为错误）。
		// 原因：1) 跟随重定向会把自定义鉴权头（x-api-key / x-goog-api-key）泄露给
		//        重定向目标——Go 仅对 Authorization/Cookie/WWW-Authenticate 做跨域剥离，
		//        不覆盖这些 provider 专用头；
		//     2) 重定向 Location 可指向内网，构成 SSRF（即便首跳 IP 公网）。
		// 健康检查的良性上游应直接 2xx 响应，不应依赖重定向。
