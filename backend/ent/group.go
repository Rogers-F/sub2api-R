...
type Group struct {
	...
	// 是否启用 AWS Bedrock 请求体严格兼容归一化
	BedrockRequestCompatEnabled bool `json:"bedrock_request_compat_enabled,omitempty"`
	...
	// 启用后，stream=false 的 JSON 响应统一返回 application/json Content-Type
	ForceApplicationJSONForNonStream bool `json:"force_application_json_for_non_stream,omitempty"`
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldBedrockRequestCompatEnabled:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field bedrock_request_compat_enabled", values[i])
			} else if value.Valid {
				_m.BedrockRequestCompatEnabled = value.Bool
			}
		...
		case group.FieldForceApplicationJSONForNonStream:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field force_application_json_for_non_stream", values[i])
			} else if value.Valid {
				_m.ForceApplicationJSONForNonStream = value.Bool
			}
		default:
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("bedrock_request_compat_enabled=")
	builder.WriteString(fmt.Sprintf("%v", _m.BedrockRequestCompatEnabled))
	builder.WriteString(", ")
	...
	builder.WriteString(", ")
	builder.WriteString("force_application_json_for_non_stream=")
	builder.WriteString(fmt.Sprintf("%v", _m.ForceApplicationJSONForNonStream))
	builder.WriteByte(')')
