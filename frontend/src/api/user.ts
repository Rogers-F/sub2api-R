...
export async function sendNotifyEmailCode(email: string): Promise<{ message: string }> {
  const { data } = await apiClient.post<{ message: string }>('/user/notify-email/send-code', {
    email
  })
  return data
}
