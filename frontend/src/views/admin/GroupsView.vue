    ...
    >
      <form id="create-group-form" @submit.prevent="handleCreateGroup" class="space-y-5">
        ...
        <div v-if="createForm.platform === 'anthropic'" class="border-t pt-4">
          <div class="mb-1.5 flex items-center gap-1">
            <label class="text-sm font-medium text-gray-700 dark:text-gray-300">
              {{ t('admin.groups.bedrockRequestCompat.title') }}
            </label>
            <div class="group relative inline-flex">
              <Icon
                name="questionCircle"
              ...
              <div class="pointer-events-none absolute bottom-full left-0 z-50 mb-2 w-80 opacity-0 transition-all duration-200 group-hover:pointer-events-auto group-hover:opacity-100">
                <div class="rounded-lg bg-gray-900 p-3 text-white shadow-lg dark:bg-gray-800">
                  <p class="text-xs leading-relaxed text-gray-300">
                    {{ t('admin.groups.bedrockRequestCompat.tooltip') }}
                  </p>
                  <div class="absolute -bottom-1.5 left-3 h-3 w-3 rotate-45 bg-gray-900 dark:bg-gray-800"></div>
                </div>
              </div>
          ...
          <div class="flex items-center gap-3">
            <button
              type="button"
              @click="createForm.bedrock_request_compat_enabled = !createForm.bedrock_request_compat_enabled"
              :class="[
                'relative inline-flex h-6 w-11 items-center rounded-full transition-colors',
                createForm.bedrock_request_compat_enabled ? 'bg-primary-500' : 'bg-gray-300 dark:bg-dark-600'
              ]"
            >
              <span
                :class="[
                  'inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform',
                  createForm.bedrock_request_compat_enabled ? 'translate-x-6' : 'translate-x-1'
                ]"
              />
            </button>
            <span class="text-sm text-gray-500 dark:text-gray-400">
              {{ createForm.bedrock_request_compat_enabled ? t('admin.groups.bedrockRequestCompat.enabled') : t('admin.groups.bedrockRequestCompat.disabled') }}
            </span>
          </div>
          <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
            {{ t('admin.groups.bedrockRequestCompat.hint') }}
          </p>
        </div>
      ...
      >
        ...
        <div v-if="editForm.platform === 'anthropic'" class="border-t pt-4">
          ...
          <div class="flex items-center gap-3">
            <button
              type="button"
              @click="editForm.bedrock_request_compat_enabled = !editForm.bedrock_request_compat_enabled"
              :class="[
                'relative inline-flex h-6 w-11 items-center rounded-full transition-colors',
                editForm.bedrock_request_compat_enabled ? 'bg-primary-500' : 'bg-gray-300 dark:bg-dark-600'
              ]"
            >
              <span
                :class="[
                  'inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform',
                  editForm.bedrock_request_compat_enabled ? 'translate-x-6' : 'translate-x-1'
                ]"
              />
            </button>
            <span class="text-sm text-gray-500 dark:text-gray-400">
              {{ editForm.bedrock_request_compat_enabled ? t('admin.groups.bedrockRequestCompat.enabled') : t('admin.groups.bedrockRequestCompat.disabled') }}
            </span>
          </div>
          <p class="mt-2 text-xs text-gray-500 dark:text-gray-400">
            {{ t('admin.groups.bedrockRequestCompat.hint') }}
...
const closeCreateModal = () => {
  ...
  createForm.bedrock_request_compat_enabled = false
...
const handleEdit = (group: AdminGroup) => {
  ...
  editForm.bedrock_request_compat_enabled = group.bedrock_request_compat_enabled ?? false
...
watch(
  ...
  (newVal) => {
    ...
    if (newVal !== 'anthropic') {
      editForm.bedrock_request_compat_enabled = false
    }
    if (!['anthropic', 'antigravity'].includes(newVal)) {
      editForm.fallback_group_id_on_invalid_request = null
    }
