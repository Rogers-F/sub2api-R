<template>
  <div class="flex flex-wrap items-center gap-3">
    ...
    <Select :model-value="filters.enterprise" class="w-44" :options="enterpriseOpts" @update:model-value="updateEnterprise" @change="$emit('change')" />
  </div>
</template>
...
import type { AdminGroup, Enterprise } from '@/types'
const props = defineProps<{ searchQuery: string; filters: Record<string, any>; groups?: AdminGroup[]; enterprises?: Enterprise[] }>()
...
const updateEnterprise = (value: string | number | boolean | null) => { emit('update:filters', { ...props.filters, enterprise: value }) }
...
const enterpriseOpts = computed(() => [
  { value: '', label: t('admin.enterprises.all') },
  { value: 'unassigned', label: t('admin.enterprises.unassigned') },
  ...(props.enterprises || []).map(e => ({ value: String(e.id), label: e.name }))
])
</script>
