<template>
  ...
  >
    ...
    >
      ...
      <!-- OpenAI Codex preset instructions 开关（OAuth/API Key） -->
      <div
        v-if="account?.platform === 'openai' && (account?.type === 'oauth' || account?.type === 'apikey')"
        class="border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div class="flex items-center justify-between">
          <div>
            <label class="input-label mb-0">{{ t('admin.accounts.openai.codexPreset') }}</label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
              {{ t('admin.accounts.openai.codexPresetDesc') }}
            </p>
          </div>
          <button
            type="button"
            data-testid="openai-codex-preset-toggle"
            @click="openAICodexPresetEnabled = !openAICodexPresetEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              openAICodexPresetEnabled ? 'bg-primary-600' : 'bg-gray-200 dark:bg-dark-600'
            ]"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                openAICodexPresetEnabled ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
      ...
      >
        <div class="flex items-center justify-between">
          ...
          <select
            ...
            data-testid="web-search-emulation-mode"
...
const openAICodexPresetEnabled = ref(false)
...
const loadWebSearchGlobalState = async () => {
  try {
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  openAICodexPresetEnabled.value = false
  ...
  if (newAccount.platform === 'openai' && (newAccount.type === 'oauth' || newAccount.type === 'apikey')) {
    ...
    openAICodexPresetEnabled.value =
      extra?.enable_codex_preset === true || extra?.openai_codex_preset_instructions === true
  ...
  if (newAccount.platform === 'anthropic' && newAccount.type === 'apikey') {
    ...
    if (extra?.web_search_emulation === 'enabled' || extra?.web_search_emulation === 'disabled') {
      webSearchEmulationMode.value = extra.web_search_emulation
    } else if (extra?.web_search_emulation === true) {
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (props.account.platform === 'openai' && (props.account.type === 'oauth' || props.account.type === 'apikey')) {
      ...
      const hadCodexPresetSetting =
        typeof currentExtra.enable_codex_preset === 'boolean' ||
        typeof currentExtra.openai_codex_preset_instructions === 'boolean'
      ...
      if (openAICodexPresetEnabled.value) {
        newExtra.enable_codex_preset = true
        delete newExtra.openai_codex_preset_instructions
      } else if (hadCodexPresetSetting) {
        newExtra.enable_codex_preset = false
        delete newExtra.openai_codex_preset_instructions
      } else {
